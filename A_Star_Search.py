...
class Fringe:
    ...
    def insert(self, i, g, h):
        heapq.heappush(self.priorityQ, (g+h, i, g, h))
...
fringe.insert(start_state,0, heuristic_value(0,N-1, ei, ej))
closed_set = set()

solved = False

while (True):
    f_value, pt, cost, h = fringe.delete()
    if (pt == -1):
        break
    ...
    if (cj - 1 >= 0) and (ci, cj - 1) not in closed_set:  # left move
        children.append((pt + [(ci, cj - 1)], cost + 1, heuristic_value(ci, cj - 1, ei, ej)))  # left move
    if (cj + 1 <= N - 1) and (ci, cj + 1) not in closed_set:  # right move
        children.append((pt + [(ci, cj + 1)], cost + 1, heuristic_value(ci, cj + 1, ei, ej)))  # right move
    if (ci - 1 >= 0) and (ci - 1, cj) not in closed_set:  # up move
        children.append((pt + [(ci - 1, cj)], cost + 2, heuristic_value(ci - 1, cj, ei, ej)))  # up move
    if (ci + 1 <= N - 1) and (ci + 1, cj) not in closed_set:  # down move
        children.append((pt + [(ci + 1, cj)], cost + 2, heuristic_value(ci + 1, cj, ei, ej)))  # down move


    # Insert all children into the fringe
    for child, cost, h_value in children:
        fringe.insert(child, cost, h_value)
        print(f"inserted node is, {mat[child[-1][0]][child[-1][1]]}, cost (g): {cost}, heuristic value (h): {h_value}, f: {cost+h_value}")
